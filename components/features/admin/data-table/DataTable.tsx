  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm animate-pulse">
        ...
        {[...Array(5)].map((_, idx) => (
          <div key={idx} className="flex items-center gap-4 p-4 border-b border-slate-50">
            ...
            <div className="flex items-center gap-1">
              {[...Array(totalPages)].map((_, i) => {
                ...
                if (totalPages > 7 && p !== 1 && p !== totalPages && Math.abs(p - page) > 1) {
                  if (p === 2 || p === totalPages - 1) return <span key={p} className="text-slate-400 text-xs px-1">...</span>;
                  return null;
                }
