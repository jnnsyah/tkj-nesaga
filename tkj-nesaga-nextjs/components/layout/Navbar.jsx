...
export default function Navbar() {
    ...
    const isActive = (path) => {
        return location === path ? "text-primary font-bold" : "text-foreground/80 hover:text-primary";
    };
        ...
        <nav className="fixed top-6 left-0 right-0 z-50 flex flex-col items-center px-4 transition-all duration-300">
            ...
            <div className="w-full max-w-4xl bg-white/80 dark:bg-background-dark/80 backdrop-blur-md border border-white/20 dark:border-white/10 shadow-lg rounded-full px-8 py-3 flex items-center justify-between relative z-50">
                ...
                <div className="hidden md:flex items-center gap-8">
                    ...
                    {location.startsWith('/prakerin') && (
                        <Link href="/prakerin/guide" className={cn("text-sm font-semibold transition-colors", isActive("/prakerin/panduan"))}>Panduan</Link>
                    )}
                </div>
            ...
            <div className={cn(
                ...
                <div className="flex flex-col p-4 gap-2">
                    ...
                    {location.startsWith('/prakerin') && (
                        <Link href="/prakerin/panduan" onClick={closeMenu} className={cn("p-3 rounded-xl hover:bg-primary/10 transition-colors text-sm font-semibold", isActive("/prakerin/panduan"))}>
                            Panduan
                        </Link>
                    )}
                </div>
